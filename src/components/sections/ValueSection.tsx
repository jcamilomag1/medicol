...
export const ValueSection = () => {
  ...
  useEffect(() => {
    if (isInView) {
      const timer = setTimeout(() => setCount(100), 200);
      return () => clearTimeout(timer);
    }
  }, [isInView]);

  const titleVariants = {
    hidden: { opacity: 0, y: -20, filter: "blur(10px)" } as const,
    visible: { 
      opacity: 1, 
      y: 0, 
      filter: "blur(0px)",
  ...
  const card1Variants = {
    hidden: { opacity: 0, scale: 0.8, rotate: -5 } as const,
    visible: { 
      opacity: 1, 
      scale: 1, 
      rotate: 0,
  ...
  const cardSlideUpVariants = (delay: number) => ({
  ...
  const iconSpinVariants = (delay: number) => ({
  ...
  const testimonialVariants = (delay: number) => ({
    hidden: { opacity: 0, scale: 0.5 } as const,
    visible: { 
      opacity: 1, 
      scale: 1,
      transition: { 
  ...
  const starVariants = (index: number, baseDelay: number) => ({
      ...
      <div className="mx-auto max-w-5xl px-6 lg:max-w-7xl">
        ...
        <div className="relative">
          <div className="relative z-10 grid grid-cols-6 gap-3">
            ...
            >
            <Card className="relative h-full flex overflow-hidden hover:scale-[1.02] transition-transform duration-300">
              <CardContent className="relative m-auto size-fit pt-6">
                <div className="relative flex h-24 w-56 items-center">
                  <motion.svg 
                    className="text-accent/30 absolute inset-0 size-full" 
                    viewBox="0 0 254 104" 
                    fill="none" 
                  ...
                  <span className="mx-auto block w-fit text-5xl font-semibold">
                    <NumberFlow value={count} format={{ notation: 'compact' }} suffix="%" />
                  </span>
                </div>
                <h2 className="mt-6 text-center text-3xl font-semibold text-primary">{t('value_proposition.reliable.title')}</h2>
            ...
            <motion.div
              initial="hidden"
              animate={isInView ? "visible" : "hidden"}
              variants={cardSlideUpVariants(0.3)}
              className="relative col-span-full sm:col-span-3 lg:col-span-2"
            >
            <Card className="relative h-full overflow-hidden hover:scale-[1.02] transition-transform duration-300">
              <CardContent className="pt-6">
                <div className="relative mx-auto flex aspect-square size-32 rounded-full border before:absolute before:-inset-2 before:rounded-full before:border dark:border-white/10 dark:before:border-white/5">
                  <motion.div
                    initial="hidden"
                    animate={isInView ? "visible" : "hidden"}
                    variants={iconSpinVariants(0.3)}
                  >
                    <ShieldCheck className="m-auto size-16 text-accent" strokeWidth={1.5} />
                  </motion.div>
                </div>
            ...
            <motion.div
              initial="hidden"
              animate={isInView ? "visible" : "hidden"}
              variants={cardSlideUpVariants(0.4)}
              className="relative col-span-full sm:col-span-3 lg:col-span-2"
            >
            <Card className="relative h-full overflow-hidden hover:scale-[1.02] transition-transform duration-300">
              <CardContent className="pt-6">
                <div className="relative mx-auto flex aspect-square size-32 rounded-full border before:absolute before:-inset-2 before:rounded-full before:border dark:border-white/10 dark:before:border-white/5">
                  <motion.div
                    initial="hidden"
                    animate={isInView ? "visible" : "hidden"}
                    variants={iconSpinVariants(0.4)}
                  >
                    <Languages className="m-auto size-16 text-accent" strokeWidth={1.5} />
                  </motion.div>
                </div>
                <div className="relative z-10 mt-6 space-y-2 text-center">
                ...
                <div className="rounded-tl-(--radius) relative -mb-6 -mr-6 mt-6 h-fit border-l border-t p-6 py-6 sm:ml-6">
                  ...
                  <svg className="w-full sm:w-[150%]" viewBox="0 0 366 231" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                      ...
                      fill="url(#paint0_linear_savings)"
                    />
                    <motion.path
                    ...
                    />
                    <defs>
                      <linearGradient id="paint0_linear_savings" x1="0.85108" y1="0.947876" x2="0.85108" y2="230.114" gradientUnits="userSpaceOnUse">
                        <stop className="text-accent/15 dark:text-accent/35" stopColor="currentColor" />
                  ...
                  <div className="before:bg-(--color-border) relative mt-6 before:absolute before:inset-0 before:mx-auto before:w-px sm:-my-6 sm:-mr-6">
                    <div className="relative flex h-full flex-col justify-center space-y-6 py-6">
                      <motion.div 
                        ...
                        initial="hidden"
                        animate={isInView ? "visible" : "hidden"}
                        variants={testimonialVariants(0.6)}
                      >
                      ...
                      <motion.div 
                        ...
                        initial="hidden"
                        animate={isInView ? "visible" : "hidden"}
                        variants={testimonialVariants(0.7)}
                      >
                      ...
                      <motion.div 
                        ...
                        initial="hidden"
                        animate={isInView ? "visible" : "hidden"}
                        variants={testimonialVariants(0.8)}
                      >
                        <div className="flex items-center gap-0.5">
                          {[...Array(5)].map((_, i) => (
                            <motion.div
